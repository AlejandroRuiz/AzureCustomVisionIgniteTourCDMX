...
namespace IgniteTourCDMX.iOS {
	/// <summary>
	/// Model Prediction Input Type
	/// </summary>
	public class MonkeyModelInput : NSObject, IMLFeatureProvider
	{
		...
		public MonkeyModelInput (CVPixelBuffer data)
		{
			if (data == null)
	...
	/// </summary>
	public class MonkeyModelOutput : NSObject, IMLFeatureProvider
	{
		static readonly NSSet<NSString> featureNames = new NSSet<NSString> (
		...
		public MonkeyModelOutput (NSDictionary<NSObject, NSNumber> loss, string classLabel)
		{
			if (loss == null)
	...
	/// </summary>
	public class MonkeyModel : NSObject
	{
		readonly MLModel model;

		static NSUrl GetModelUrl ()
		{
			return NSBundle.MainBundle.GetUrlForResource ("MonkeyModel", "mlmodelc");
		}
		...
		MonkeyModel (MLModel model)
		{
			this.model = model;
		}

		public static MonkeyModel Create (NSUrl url, out NSError error)
		{
			if (url == null)
			...
			return new MonkeyModel (model);
		}

		public static MonkeyModel Create (MLModelConfiguration configuration, out NSError error)
		{
		...
		public static MonkeyModel Create (NSUrl url, MLModelConfiguration configuration, out NSError error)
		{
			if (url == null)
				throw new ArgumentNullException (nameof (url));
		...
		/// </summary>
		/// <param name="input">an instance of MonkeyModelInput to predict from</param>
		/// <param name="error">If an error occurs, upon return contains an NSError object that describes the problem.</param>
		public MonkeyModelOutput GetPrediction (MonkeyModelInput input, out NSError error)
		{
			if (input == null)
			...
			return new MonkeyModelOutput (lossValue, classLabelValue);
		}
		...
		/// <param name="error">If an error occurs, upon return contains an NSError object that describes the problem.</param>
		public MonkeyModelOutput GetPrediction (MonkeyModelInput input, MLPredictionOptions options, out NSError error)
		{
			if (input == null)
				throw new ArgumentNullException (nameof (input));
		...
		/// <param name="error">If an error occurs, upon return contains an NSError object that describes the problem.</param>
		public MonkeyModelOutput GetPrediction (CVPixelBuffer data, out NSError error)
		{
			var input = new MonkeyModelInput (data);
		...
		/// <param name="options">prediction options</param>
		/// <param name="error">If an error occurs, upon return contains an NSError object that describes the problem.</param>
		public MonkeyModelOutput GetPrediction (CVPixelBuffer data, MLPredictionOptions options, out NSError error)
		{
			var input = new MonkeyModelInput (data);
